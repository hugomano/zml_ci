...
const Layer = struct {
    ...
    pub fn forward(self: Layer, _: zml.Tensor) zml.Tensor {
        var y = self.weight;
        if (self.bias) |bias| {
            y = y.dot(bias, .{.c});
        }
...
pub fn asyncMain() !void {
    ...
    var weights = [_]f32{ 1, 2, 3, 4, 5, 6, 7, 8 };
    var bias = [_]f32{ 1, 0, 0, 1, 1, 0, 0, 1 };
    const weights_shape = zml.Shape.init(.{ .a = 2, .b = 2, .c = 2 }, .f32);
    const bias_shape = zml.Shape.init(.{ .a = 2, .c = 2, .d = 2 }, .f32);
    ...
    try buffers.put(arena, "weight", zml.HostBuffer.fromSlice(weights_shape, &weights));
    try buffers.put(arena, "bias", zml.HostBuffer.fromSlice(bias_shape, &bias));
